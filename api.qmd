---
title: "Extension API"
description: "Complete reference for Airport extension API functions including airport_flights, airport_take_flight, airport_action, and catalog integration with DuckDB."
---
...
::: {.callout-tip}
Calling `airport_flights` and `airport_take_flight` are considered a lower level interface. Airport also [integrates with the DuckDB catalog](catalog_integration.qmd) that allows Arrow Flights to operate much like DuckDB Tables.
:::
...
This is a table returning function[^1] that produces a list of Arrow Flights that are available at the server specified.  Optionally criteria can be supplied that the server can use for filtering.
...
| criteria | `VARCHAR` | This is free-form criteria to pass to the Flight server that may be used to filter the list of returned flights. |
...
This is a scalar function that returns the current Airport user agent header value.  This value is sent to all Arrow Flight servers so that they can determine if they are compatible with this version of the Airport extension.
...
The server determines the schema it returns for the `__databases` flight, but it is recommended that it follow this schema.
