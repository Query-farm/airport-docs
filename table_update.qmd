---
title: "Update Rows"
description: "Update rows in Airport-managed tables using standard SQL UPDATE statements with RETURNING clause support for modified values."
---

The Airport extension supports updating rows in managed tables through standard SQL `UPDATE` statements. The Arrow Flight server is responsible for storing and persisting the modified data.
...
::: {.callout-tip}
Airport-managed tables lack the transactional guarantees of native DuckDB tables. When using the `UPDATE` statement, all rows are sent to the Arrow Flight server where they are presumed to be immediately committed to storage. This differs from a standard SQL transaction that doesn't commit data until `COMMIT` or `ROLLBACK` is issued.
...
If the table has a [rowid pseudocolumn](table_row_ids.qmd) the update can be performed.

A `DoExchange` Arrow Flight RPC is made to update rows. This allows the server to return rows (needed for the `RETURNING` clause), which may differ from the input.  On the write stream of the `DoExchange` call only the rowid pseudocolumn will be sent to the server along with all of the new column values.
