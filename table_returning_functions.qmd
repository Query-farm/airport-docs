...
description: "Implement table-valued functions (TVFs) in Arrow Flight servers that return multiple rows with dynamic schemas based on input parameters."
---

The Airport extension enables Arrow Flight servers to provide table-returning functions (also known as table-valued functions or TVFs). These SQL functions return an arbitrary number of rows conforming to a schema, which can be dynamically determined based on the function's input parameters.
...
For information about how to register an Arrow Flight as a table returning function refer to [Server Catalog Integration](server_catalog_integration.qmd).

### Supporting the `ANY` DuckDB Data Type

If you wish to use the DuckDB ANY datatype, create a standard Arrow field in a schema, but add additional metadata with the name of `is_any_type` with a non-empty value.  The Airport extension will treat that column as being defined as the `ANY` type.
