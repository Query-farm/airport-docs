...
Checks are specified as a set of string based expressions.  An example check expression could be `"color in ('blue', 'green')"`, that would restrict the value of the color field to be either "blue" or "green".  These checks are serialized using a `msgpack` structure, then stored in a metadata key called `check_constraints`:
