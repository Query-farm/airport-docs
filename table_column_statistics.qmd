...
DuckDB's tables can supply optional statistics that assist efficient query execution. What these statistics consist of vary based on the data type of the column.  These statistics are optional.  Tables provided via the Airport extension can provide these statistics if they choose to.

For an Airport based table to provide statitics, the Arrow schema of the table should contain a metadata key called `can_produce_statistics` with a non-empty string value.  If the table has opted-in to providing statistics an Arrow Flight `DoAction` RPC call with be made to the server with an action name of `get_flight_column_statistics`.
