...
To fetch data from a flight, the Airport extension performs a `DoAction` RPC call named `get_flight_endpoints`, instead of the standard `GetFlightInfo` RPC call. This is done because:
...
Each endpoint contains one or more locations where the data resides. Arrow Flight locations usually point to servers capable of serving data via the `DoGet` RPC call, but the Airport extension adds additional funcitonality.
...
This mechanism allows DuckDB to treat function calls (e.g., reading CSV, Parquet, Iceberg, or Delta Lake files) as Flight endpoint sources.
