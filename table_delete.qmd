...
A `DoExchange` Arrow Flight RPC call is made to delete rows. This allows the server to return rows (needed for the `RETURNING` clause), the rows may differ from the original input supplied.  On the write stream of the `DoExchange` call only the rowid pseudocolumn will be sent to the server.
