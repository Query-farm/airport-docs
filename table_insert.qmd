...
A `DoExchange` Arrow Flight RPC call is made to insert rows. This allows the server to return rows (needed for the `RETURNING` clause), the rows may differ from the original input supplied.
